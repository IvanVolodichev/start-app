...
<x-app-layout>
    <form method="POST" action="{{ route('events.store') }}" enctype="multipart/form-data" class="max-w-2xl mx-auto bg-white rounded-xl shadow-md p-6 my-6 space-y-6">
        @csrf

        <h2 class="text-3xl font-bold text-gray-800 text-center">Создание нового события</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" id="max_participant" name="max_participant" min="2" value="{{ old('max_participant') }}" required 
        ...
        <div>
            ...
            <textarea id="comment" name="comment" rows="3"
                    ...
                    placeholder="Опишите детали события">{{ old('comment') }}</textarea>
        </div>
        ...
        <!-- Кнопка отправки -->
        <button type="submit" 
                class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-200 transform hover:scale-101">
            Создать событие
        </button>
    </form>
</x-app-layout>
...
<script type="text/javascript">
        ...
        function init() {
            ...
            function updateAddress(coords) {
                ...
                // Выполняем геокодирование (без skip, format, apikey)
                ymaps.geocode(coords, {
                    // kind: 'street', // можно указать, если нужен именно поиск улицы
                    results: 1
                }).then(function(res) {
                    ...
                    // Берём первый (наиболее точный) объект
